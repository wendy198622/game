function EZSlots(id, useroptions) {
    ...
    this.init = function() {
        ...
        for (var i = 0; i < this.reelCount; i++) {
            ...
            jqoWindow.css("height", this.height * 1.7 + "px").css("width", this.width + "px").append(jqoSlider);
            this.jqo.append(jqoWindow); //add window to main div
            this.jqoSliders.push(jqoSlider); //keep reference to jqo of slider
            this.addFirstSymbolsToStrip(jqoSlider, i, false, true); //and add the initial set 
        }
    };
    ...
    this.addFirstSymbolsToStrip = function(jqoSlider, whichReel, shouldWin, isInitialCall) {
        ...
        for (var i = 0; i < that.howManySymbolsToAppend; i++) {
            ...
            if (i == 1 && isInitialCall && that.startingSet) {
                ...
                that.scaleJqo(jqoSymbol);
                jqoSymbol.append(jqoContent);
                if (i == 0) jqoSymbol.css("margin-top", "-145px")
                jqoSlider.append(jqoSymbol);
            }
    ...
    this.spinOne = function(jqoSlider, whichReel, shouldWin) {
        ...
        jqoSlider.stop(true, true).animate({ "margin-top": (marginTop + 25) + "px" }, {
            'duration': that.time + 1500 * whichReel /*Math.round(Math.random() * 1000)*/ ,
            'easing': "easeOutCubic"
        });
        return chosen;
    }
